...
- 最终报告：[cloud_course_design_report.docx](cloud_course_design_report.docx)
...
outputs/                   数据分析输出结果和图表
cloud_course_design_report.docx  最终课程设计报告
```
...
最终提交材料以报告为主，代码仓库用于支撑报告中的构建、部署、Spark 分析和附加题实现。报告中的云端验收截图已经嵌入 `cloud_course_design_report.docx`，仓库中不提交真实云账号密钥或临时登录口令。
